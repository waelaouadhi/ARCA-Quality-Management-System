...
describe('UserService', () => {
  ...
  it('throws not found for missing user', async () => {
    ...
    await expect(service.getUserById('ccccccccccccccccccccccccc', adminUser)).rejects.toMatchObject({ statusCode: 404 });
  });
  ...
  it('rejects delete for non-admin user', async () => {
    ...
    await expect(service.deleteUser('cdddddddddddddddddddddddd', normalUser)).rejects.toThrow('Admin access required');
  });
  ...
  it('deletes user for admin', async () => {
    ...
    const result = await service.deleteUser('ceeeeeeeeeeeeeeeeeeeeeeee', adminUser);

    expect(repository.deleteUser).toHaveBeenCalledWith('ceeeeeeeeeeeeeeeeeeeeeeee');
